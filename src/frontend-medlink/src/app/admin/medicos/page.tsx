    ...
    <div className={styles.medicos}>
      <div className="container">
        <header className={styles.medicos__header}>
          ...
          <div className={styles.medicos__actions} role="toolbar" aria-label="Ações">
            <button
              type="button"
              onClick={() => refetch()}
              disabled={isFetching}
        ...
        {isError && <p className={`${styles.medicos__info} ${styles['medicos__info--error']}`} role="alert">Erro ao carregar médicos.</p>}

        {data && data.length === 0 && (
          <div className={styles.medicos__empty}>
            <p className={styles.medicos__emptytext}>Nenhum médico cadastrado.</p>
            <Link href="/admin/medicos/novo" className={`${styles.btn} ${styles['btn--primary']} ${styles['btn--mt']}`}>
        ...
        {data && data.length > 0 && (
          <div className={styles.medicos__tablewrap}>
            <table className={styles.medicos__table} aria-label="Lista de médicos">
              <thead>
                <tr>
