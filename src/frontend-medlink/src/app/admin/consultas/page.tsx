  ...
  return (
    <div className={styles['admin-consultas']}>
      <div className="container">
        ...
        {/* Filtros */}
        <section className={styles['admin-consultas__filters']} aria-label="Filtros">
          <div className={`${styles['admin-consultas__field']} ${styles['admin-consultas__field--span2']}`}>
            <label htmlFor="buscar" className={styles['ac-label']}>Buscar (livre)</label>
            <input
              id="buscar"
              value={q}
          ...
          <div className={styles['admin-consultas__field']}>
            <label htmlFor="de" className={styles['ac-label']}>De</label>
            <input
              id="de"
              type="date"
              value={from}
          ...
          <div className={styles['admin-consultas__field']}>
            <label htmlFor="ate" className={styles['ac-label']}>Até</label>
            <input
              id="ate"
              type="date"
              value={to}
          ...
          <div className={styles['admin-consultas__field']}>
            <label htmlFor="status" className={styles['ac-label']}>Status</label>
            <select
              id="status"
              value={status}
          ...
          <div className={styles['admin-consultas__actions']}>
            <button type="button" onClick={applyFilters} className={`${styles['ac-button']} ${styles['ac-button--primary']}`}>
              Aplicar filtros
            </button>
            <button type="button" onClick={clearFilters} className={styles['ac-button']}>
              Limpar
            </button>
          </div>
        </section>
        ...
        <div className={styles['admin-consultas__tablewrap']}>
          <table className={styles['ac-table']} aria-label="Lista de consultas">
            ...
            <tbody>
              {isLoading && (
                <tr>
                  <td className={styles['ac-cell']} colSpan={6}>Carregando...</td>
                </tr>
              )}
              {isError && (
                <tr>
                  <td className={`${styles['ac-cell']} ${styles['ac-cell--error']}`} colSpan={6}>
              ...
              {!isLoading && (pageItems ?? []).length === 0 && (
                <tr>
                  <td className={styles['ac-cell']} colSpan={6}>Nenhuma consulta encontrada.</td>
                </tr>
              )}

              {(pageItems ?? []).map((c) => (
                <tr key={c.id}>
                  <td className={styles['ac-cell']}>{formatDateTime(c.dataHora)}</td>
                  <td className={styles['ac-cell']}>
                    <div className={styles['ac-strong']}>{pacienteLabel(c.pacienteId)}</div>
                  </td>
                  <td className={styles['ac-cell']}>
                    <div className={styles['ac-strong']}>{medicoLabel(c.medicoId)}</div>
                  ...
                  <td className={styles['ac-cell']}>
                    <span
                      className={[
                        styles['ac-chip'],
                        c.status === 'CONFIRMADO' ? styles['ac-chip--ok'] :
                        c.status === 'CANCELADO' ? styles['ac-chip--danger'] :
        ...
        <div className={styles['admin-consultas__pagination']}>
          <div className={styles['ac-muted']}>Página {page + 1} de {Math.max(totalPages, 1)}</div>
